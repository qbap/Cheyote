...
//  cheyote
//
//  Created by 24 Aaron on 28/02/2022.
//

import UIKit
import MachO.dyld

class ViewController: UIViewController, ChimeraUI {
    ...
    @IBOutlet weak var updateTaurineView: UIVisualEffectView!
        ...
        */
        ...
        if getSafeEntitlements().count < 3 {
            self.showAlert("Sanity Check Failed",
                           "Your copy of Cheyote has not been signed correctly. Please sign it properly to continue.",
                           sync: false)
            jailbreakButton.isEnabled = false
        ...
        updateTaurineView.addGestureRecognizer(updateTapGestureRecogniser)
                ...
                if (updateRequired) {
                    DispatchQueue.main.async {
                        UIView.animate(withDuration: 0.5) {
                            self.updateTaurineView.isHidden = false
                        }
                    }
                }
            }
    ...
    @IBAction func jailbreak() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            DispatchQueue.global(qos: .userInteractive).async {
                ...
                if #available(iOS 15, *){
                    print("Selecting multicast_bytecopy for iOS 15.0 - 15.1.1")
                    if multicast_bytecopy() == 0 {
    ...
    @IBAction func openDiscord(){
        UIApplication.shared.open(URL(string: "https://cheyote.app/discord")!, options: [:], completionHandler: nil)
    }
